...
(function() {
  ...
  preProcessMessage = function(dataSet) {
    var attrs, comp, dims, ext, _base, _i, _len, _ref, _ref1, _ref2, _ref3;
    ext = (_ref = (_base = dataSet.structure).extensions) != null ? _ref : _base.extensions = {};
    dims = [];
    dims.push.apply(dims, dataSet.structure.packaging.dataSetDimensions);
    dims.push.apply(dims, dataSet.structure.packaging.seriesDimensions);
    dims.push.apply(dims, dataSet.structure.packaging.observationDimensions);
    attrs = [];
    attrs.push.apply(attrs, dataSet.structure.packaging.dataSetAttributes);
    attrs.push.apply(attrs, dataSet.structure.packaging.seriesAttributes);
    attrs.push.apply(attrs, dataSet.structure.packaging.observationAttributes);
    ext.keyDimensions = [];
    ext.attributes = {};
    _ref1 = dataSet.structure.components;
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      comp = _ref1[_i];
      if ((_ref2 = comp.id, __indexOf.call(dims, _ref2) >= 0) && comp.id !== 'TIME_PERIOD') {
        ext.keyDimensions.push(comp);
      }
      if (_ref3 = comp.id, __indexOf.call(attrs, _ref3) >= 0) {
        ext.attributes[comp.id] = comp;
      }
    }
    dataSet.structure.extensions.timeDimension = ext.dimensions['TIME_PERIOD'];
    return dataSet;
  };
  ...
  filterTimePeriods = function(request, response, msg) {
    ...
    periods = [];
    if (msg.structure.extensions.timeDimension != null) {
      _ref = msg.structure.extensions.timeDimension.codes.id;
  ...
  addCodesToQuery = function(request, response, msg) {
    var code, codeMap, codePositions, dim, i, j, key, keyCodes, query, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref, _ref1, _ref2, _ref3;
    query = [];
    ...
    if (request.query.key === 'all') {
      for (_i = 0, _len = key.length; _i < _len; _i++) {
        ...
        for (j = _j = 0, _len1 = _ref.length; _j < _len1; j = ++_j) {
          code = _ref[j];
          codePositions.push(j);
        }
        query.push(codePositions);
      }
      return query;
    }
    ...
    for (i = _k = 0, _len2 = _ref1.length; _k < _len2; i = ++_k) {
      ...
      for (_n = 0, _len5 = keyCodes.length; _n < _len5; _n++) {
        code = keyCodes[_n];
        codePositions.push(codeMap[code] != null ? codeMap[code] : -1);
      }
      query.push(codePositions);
    }
    return query;
  ...
  query = function(msg, request, response) {
    var attrId, attributesInResults, code, codeCounts, codesInQuery, codesWithData, compressDataSet, counter, dim, filterDataSet, filterSeriesObservations, filterTime, filtered, formatDataSet, i, isInQuery, isInQueryTime, mapDataSetToResult, newObs, newPos, noAttributes, noObservations, obj, obj2, obs, obsCount, obsDimPos, observationInQuery, observations, pos, resultDataSets, resultObj, results, results2, rslt, timeCodes, _base, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _len9, _m, _n, _o, _p, _q, _r, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _results;
    ...
    filterDataSet = function(dataSet) {
      var dimId, i, value, _i, _len, _ref1, _ref2;
      ...
      for (i = _i = 0, _len = _ref1.length; _i < _len; i = ++_i) {
        value = _ref1[i];
        dimId = msg.structure.packaging.dataSetDimensions[i];
        if (_ref2 = !value, __indexOf.call(codesInQuery[dimId], _ref2) >= 0) {
          return false;
        }
      }
      return true;
    };
    filterSeriesObservations = function(series) {
      return series;
    };
    observationInQuery = function(obs) {
      return obs;
    };
    mapDataSetToResult = function(dataSet) {
      ...
      if (dataSet.series != null) {
        result.series = dataSet.series.filter(seriesInQuery).map(filterSeriesObservations);
      }
      if (dataSet.observations != null) {
        result.observations = dataSet.observations.filter(observationInQuery);
      }
      return result;
    };
  ...
  test = function() {
    ...
    req = {
      method: 'GET',
      url: 'data/ECB_ICP1/M.PT.N.073000.4.INX?startPeriod=2007&endPeriod=2009',
      headers: {
